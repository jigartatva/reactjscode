...
const Model = React.createClass({
    ...
    duplicate(e) {
        ...
        var existingModelsName = this.props.modellist;
        var regExCountDuplicates = /\([0-9]+\)$/; //tatva-sh
        var currentModelName = this.props.model.name.replace(regExCountDuplicates, '').trim(); //tatva-sh
        var match = this.props.model.name.match(regExCountDuplicates);
        if (match) {
            matchedElements.push(parseInt(match[0].replace(/\(/, "").replace(/\)/, "")));
        }
        var matchedCount = _.countBy(existingModelsName, function (modelname) {//tatva-sh
            var thisModelName = modelname.name.replace(regExCountDuplicates, '').trim();
        ...
        _.delay(() => {
            var newName = currentModelName + " (" + (_.max(matchedElements) + 1) + ")";
